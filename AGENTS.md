...
- **Architectural Evaluation Protocol**: Before making decisions or starting tasks, explicitly analyze and discuss what we are doing, why, pros/cons, and alternatives in chat. Agree on the trade-offs before proceeding.
- **Inter-Service Communication Trade-Offs (HTTP vs. RabbitMQ/Message Broker)**:
  - **Synchronous REST (HTTP)**: User Login / Authentication (Frontend to `auth_service`), synchronous data queries (Service-to-Service reads where the caller requires an immediate response).
  - **Asynchronous Message Broker (RabbitMQ / Queue)**: Background side-effects and decoupled workflows, such as Workspace Provisioning on signup (`auth_service` ➡️ RabbitMQ ➡️ `workspace_service`), email notifications, and asynchronous state synchronization.
