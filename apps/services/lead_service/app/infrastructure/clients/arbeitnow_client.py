"""
Resilient asynchronous client for the Arbeitnow Job Board API.

Design Patterns:
- Dependency Injection: Client accepts an optional externally managed httpx.AsyncClient.
- Async Context Manager: Enforces explicit lifecycle management for network sockets.
- Retry with Exponential Backoff: Tenacity decorator handles transient upstream failures.
- Typed Domain Exception Wrapping: Translates HTTP/Network errors to bounded domain exceptions.
"""
...
logger = logging.getLogger("lead_service.arbeitnow_client")


class ArbeitnowClient:
    """Production-grade async client for fetching opportunities from the Arbeitnow API."""

    PROVIDER_NAME = "arbeitnow"
    DEFAULT_BASE_URL = "https://www.arbeitnow.com/api"
    ...
    async def _send_request(self, endpoint: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                logger.warning(
                    "Rate limit encountered on Arbeitnow API",
            ...
            if 500 <= response.status_code < 600:
                logger.warning(
                    "Upstream server error on Arbeitnow API",
            ...
            if response.status_code >= 400:
                logger.error(
                    "Client error on Arbeitnow API",
                    extra={"url": url, "status_code": response.status_code, "duration_ms": duration_ms},
                )
                raise DiscoveryClientError(
                    message=f"Arbeitnow API responded with HTTP {response.status_code}",
                    status_code=response.status_code,
                    details={"url": url, "body": response.text[:200]},
                )

            logger.info(
                "Arbeitnow API request successful",
                extra={"url": url, "status_code": response.status_code, "duration_ms": duration_ms},
            )
            return response.json()

        except httpx.TimeoutException as exc:
            logger.warning("Timeout connecting to Arbeitnow API: %s", str(exc))
            raise DiscoveryTimeoutError(
                provider=self.PROVIDER_NAME,
                timeout_seconds=self._timeout_config.read or 15.0,
            ) from exc
        except httpx.NetworkError as exc:
            logger.warning("Network connection error to Arbeitnow API: %s", str(exc))
    ...
    async def fetch_jobs(self, page: int = 1) -> list[ArbeitnowJobItem]:
        """
        Fetch job opportunities from Arbeitnow API.
        """
        params: dict[str, Any] = {"page": page} if page > 1 else {}
        data = await self._send_request("job-board-api", params=params if params else None)

        try:
            envelope = ArbeitnowResponse.model_validate(data)
            return envelope.data
        except ValidationError as exc:
            raise MalformedPayloadError(
                provider=self.PROVIDER_NAME,
                reason=f"Invalid Arbeitnow jobs schema: {exc}",
            ) from exc
